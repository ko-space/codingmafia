...
const TASK_BANK = {
  html: [
    ...
    {prompt:"다음 중 기본 체위가 아닌 것은?", choices:["정상위","후배위","굴곡위","기승위"], answer:2, delta:5},
  ],
  css: [
    ...
    {prompt:"다음 중 변형 체위가 아닌 것은?", choices:["아마존","들박","신장위","입위"], answer:3, delta:4}
  ],
  js: [
...
const TASK_BANK_MAFIA = {
  html: [
    {prompt:"WAI-ARIA의 aria-label 주요 목적?", choices:["색상 지정","배치 제어","접근성 향상","폰트 로딩"], answer:2, delta:7},
    {prompt:"<picture>와 <source> 조합의 주된 용도", choices:["반응형 이미지 소스 전환","애니메이션","벡터 드로잉","스트리밍"], answer:0, delta:6},
    {prompt:"<template> 태그의 특징", choices:["DOM에 즉시 렌더","스크립트만 가능","비활성 DOM 조각","서버 전용"], answer:2, delta:7},
    {prompt:"<meta viewport>의 의미", choices:["인코딩 지정","모바일 뷰포트 스케일/폭 지정","색상 모드","쿠키 정책"], answer:1, delta:6},
    {prompt:"<link rel='preload'>의 역할", choices:["지연 로딩","우선 프리로드","캐시 무효화","서비스워커"], answer:1, delta:7},
    {prompt:"콘텐츠의 주 의미 영역을 지정하는 태그", choices:["<section>","<aside>","<main>","<nav>"], answer:2, delta:6},
    {prompt:"<figure>/<figcaption> 관계", choices:["표 데이터","코드 블록","폼 그룹","이미지와 캡션 묶음"], answer:3, delta:8},
    {prompt:"시맨틱 내비게이션 영역", choices:["<navigate>","<menu>","<nav>","<path>"], answer:2, delta:8},
    {prompt:"스크린리더에만 보이게 숨길 때 흔한 기법은?", choices:["display:none","visibility:hidden","sr-only 유틸리티","opacity:0"], answer:2, delta:6},
    {prompt:"서버 전송 전 입력 값 검증 우선 위치", choices:["서버만","클라만","클라+서버 모두","아무데나"], answer:2, delta:8},
  ],
  css: [
    {prompt:":root에서 선언한 --brand를 쓰는 올바른 문법", choices:["var(--brand)","root(--brand)","--brand()","use(--brand)"], answer:0, delta:6},
    {prompt:"Grid에서 repeat(3, 1fr)의 의미", choices:["3행 자동","3열 동일 분배","3칸 겹침","3배수 간격"], answer:1, delta:7},
    {prompt:"calc(100% - 48px)의 용도", choices:["수치 연산하여 동적 크기","반응형 그림자","변수 선언","정적 고정"], answer:0, delta:8},
    {prompt:"contain: layout; 의 주 효과", choices:["자식 배치 격리로 성능 향상","GPU 강제","투명도 제어","애니메이션 가속"], answer:0, delta:6},
    {prompt:"will-change 사용 시 주의점", choices:["언제나 남발","모바일 금지","IE만 사용","필요한 곳만 제한적으로"], answer:3, delta:8},
    {prompt:"clamp( min, preferred, max ) 의미", choices:["3단계 색상","폰트 합성","값을 범위로 클램프","그리드 자동"], answer:2, delta:6},
    {prompt:"@supports의 목적", choices:["브라우저 버전 검출","기능 지원 여부 분기","해상도 체크","컬러 프로파일"], answer:1, delta:6},
    {prompt:"prefers-color-scheme는 무엇?", choices:["접근성 폰트","다크모드 선호 미디어쿼리","고대비 테마","고정 팔레트"], answer:1, delta:8},
    {prompt:"BEM 네이밍에서 Block__Element--Modifier 예", choices:["btn__icon--large","btn.icon.large","btn-icon-large","btn:icon:large"], answer:0, delta:6},
    {prompt:"line-height 단위 없는 값의 의미", choices:["px","em","배수","%"], answer:2, delta:8},
  ],
  js: [
    {prompt:"클로저(closure)가 형성되는 조건", choices:["함수 내부에서 외부 스코프 변수 참조","객체 메서드만","배열 순회일 때","화살표 함수만"], answer:0, delta:7},
    {prompt:"이벤트 루프에서 마이크로태스크 큐에 들어가는 것은?", choices:["setTimeout","requestAnimationFrame","Promise.then","setInterval"], answer:2, delta:7},
    {prompt:"this 바인딩이 호출 방식에 따라 달라지는 이유", choices:["렉시컬 고정","호출 컨텍스트 기반","파일 경로 기준","브라우저만 다름"], answer:1, delta:6},
    {prompt:"async 함수는 무엇을 반환?", choices:["값 자체","undefined","Iterator","Promise"], answer:3, delta:6},
    {prompt:"debounce와 throttle의 차이", choices:["둘 다 동일","디바운스=마지막 1회, 스로틀=주기적 제한","반대이다","디바운스=주기, 스로틀=마지막"], answer:1, delta:6},
    {prompt:"深 복사(Deep Copy)로 안전한 방식", choices:["obj2 = obj1","Object.assign 깊은 복사","= 만 사용","JSON.parse(JSON.stringify(obj)) 제한적"], answer:3, delta:8},
    {prompt:"Map과 Object 차이로 옳은 것은?", choices:["키 타입 제한 동일","둘 다 동일","Object가 순서 안전","Map은 키 타입 제한 없음/순서 유지 우수"], answer:3, delta:8},
    {prompt:"이벤트 캡처링 단계에서 리스너 등록 옵션", choices:["useCapture:true","passive:true","once:true","stop:true"], answer:0, delta:8},
    {prompt:"모듈 스코프에서의 최상위 this는?", choices:["window","globalThis","undefined(모듈)","document"], answer:2, delta:6},
    {prompt:"WeakMap의 주된 장점", choices:["키가 GC 대상이어도 참조 유지","키가 GC되면 자동 해제","순회가 쉬움","직렬화에 유리"], answer:1, delta:6},
  ],
};
